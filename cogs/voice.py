...
class voice(commands.Cog):
            ...
            try:
                if after.channel.id == voiceID:
                    ...
                    if cooldown is None:
                        ...
                        await member.send("創建頻道的速度太快了!請等待15秒後再試!")
    ...
    async def helped(self, ctx):
        ...
        embed.set_footer(text='Bot developed by AD#5438')
    ...
    async def setup(self, ctx):
        ...
        if ctx.author == ctx.guild.owner or ctx.author.id == 881312396784840744:
            ...
            await ctx.channel.send("**您有 60 秒的時間來回答每個問題！**")
            await ctx.channel.send(f"**輸入類別名稱：（例如:語音頻道）**")
            try:
                category = await self.bot.wait_for('message', check=check, timeout = 60.0)
            except asyncio.TimeoutError:
                await ctx.channel.send('時間到!你沒有完成所有設定，請重新再式一次')
            else:
                new_cat = await ctx.guild.create_category_channel(category.content)
                await ctx.channel.send('**輸入頻道的名稱 : (例如:按我創建語音頻道)**')
                try:
                    ...
                    try:
                        ...
                        await ctx.channel.send("**設定完成!**")
                    except:
                        await ctx.channel.send("糟糕!發生了一些問題")
        else:
            await ctx.channel.send(f"{ctx.author.mention} 你沒有使用這個指令的權限")
        conn.commit()
        conn.close()
    ...
    async def lock(self, ctx):
        ...
        if voice is None:
            ...
            await ctx.channel.send(f'{ctx.author.mention} 上鎖成功! 🔒')
        conn.commit()
        conn.close()
    ...
    async def unlock(self, ctx):
        ...
        if voice is None:
            ...
            await ctx.channel.send(f'{ctx.author.mention} 解鎖成功! 🔓')
        conn.commit()
        conn.close()
